...
const terminateTcpdump = (inFile, outFile) => {
  ...
  const packets = pcap_parser.parsePcapFile(inFile);
  ...
  // Write to file
  fs.writeFileSync(outFile, JSON.stringify(ciData));
};
...
const main = () => {
  ...
  if (startTcpdump && !fs.existsSync(filePcap)) {
    ...
    terminateTcpdump(filePcap, outputFile);
  } else {
